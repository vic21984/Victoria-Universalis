...
## Gameplay changes:
* Introduced new Victoria 3 like production system. All tradeable goods are produced in buildings instead of at the province level, and this includes resources such as grain and coal. These new production buildings have a level cap of 5 can only be built in a state that has the corresponding RGO, e.g. an iron mine building can only be built in a state that has an iron RGO.
* Farmers are now Peasants and produce non-tradeable subsistence goods that provide them with a small income but that cannot be used in buildings or sold to other POPs. Subsistence goods main purpose is to restrict where these new resource buildings can be built.
* Overall number of goods was decreased to make room for the new RGO buildings. Some goods were removed and others were combined into one good.
* Reforms are now passed via a new political clout system. Every month, countries generate liberal, social, and reactionary clout based on their upper house, ruling party, political/social movement support, Political Thought and Psychology tech level. When enacting new reforms, countries need either 100 liberal or social clout and revoking reforms usually requires 100 reactionary clout. The requirements to pass a reform can be seen by hovering the cursor over the desired reform. Clout values can be checked via the province selector menu.
* Added several new political and social reforms.
* Party policies no longer grant any effects, instead they determine whether you can pass certain political reforms.
* Introduced Workforce Ratio mechanic that represents the percentage of peasants that are allowed to promote, e.g. a peasant pop in a country with 40% workforce ratio means that only 40% of the peasants in that province can promote, this means they will no longer promote into labourers if they fall below 60% of the total pops in their state. Workforce ratio is increased by Women's Rights political reforms and is decreased by Child Labor and Unemployment reforms. Workforce Ratio can be checked via the province selector menu.
* Removed colonial states and uncivilized status. Countries that were uncivilized are instead "Unrecognized" and no longer gain research points from conquest, start with the Closed Borders reform, cannot change government, and have Unrecognized triggered modifier that reduces prestige to 0. The "Become Westernized" decision removes the Unrecognized modifier, adds the "Rapid Industrialization" modifier that temporarily increases promotion rate and gives 100 liberal clout.
* Base bureaucrat percentage for 100% administration efficiency lowered to 0.5%. National focus techs and most social reforms now raise the minimum bureaucrat percentage which greatly increases bureaucracy cost as the game progresses.
* Made non-accepted pops able to promote to Bureaucrats in order to allow for nations to have enough bureaucracy after conquering a lot of non-accepted pops.
* Healthcare is no longer funded by the social spending slider.
* Promotion national focuses have been replaced by 2 new national focuses that give a bonus to labourers or soldiers. Their bonuses scale inversely proportional to the population of a state and are halved in non-core states, e.g. a cored state with 200k population will have a 20x bigger bonus than a cored state with 4 million population and a 40x bigger bonus than a non-core state with 4M population.
* The ideology & party loyalty focuses gives a +200% chosen ideology boost your pops in addition to influencing party loyalty. Previously these focuses only boosted party loyalty but didn't change ideology.
* Added a research point bonus to the education slider and a promotion bonus to the administration slider.
* Clerks were replaced with a new pop type that gives research points called Intellectuals.
* Research points are now generated by Intellectuals, Clergymen, and Capitalists. Intellectuals cost 4x more upkeep than Clergymen. The max cap of Intellectuals and Clergymen is determined by Religious Laws and Social Thought tech level and they will quickly automatically promote to their max cap without any player intervention. The max cap of Capitalists is 1.5% and their rate of promotion is determined by Economic Laws, Trade Laws, Land Reform laws, Financial Institutions tech level, and the presence of a Stock Exchange in their state.
* Introduced Stock Exchange building that employs capitalists as workers and produces stocks, which are consumed by rich POPs. To build a Stock Exchange, first get the tech and wait to invent "Regulated Buying & Selling of Stocks", then build a province selector in a province in the state you want to build the Stock Exchange, and click the option to "Establish Financial District". This will allow you to build a Stock Exchange in that state. A country can only have 1 Stock Exchange unless they gain another one through conquest.
...
* Soldier cap in provinces below 10k raised to 15% and soldier cap in provinces between 10k and 20k raised to 10%. This is to allow small provinces enough soldiers to reach the minimum 1000 soldier pop requirement to raise a brigade.
* Revanchism no longer raises max soldier cap but it still provides a mobilization boost.
* Lowered oil consumption of Tanks and Airplane units.
* Added War Economy decision that lets players choose between Partial and Total War Economy. Partial War Econ gives a 50% bonus to mil goods throughput and a 25% malus to consumer goods throughput, Total War Econ gives 100% mil goods bonus and 50% consumer goods malus.
* Removed the World Bank. All goods are now subject to supply and demand price mechanics.
* Enabling Core Integration no longer disables the vanilla coring event.


## Country changes:
###### Balkans:
* Created Bulgaria-Romania formable country.
* Form Byzantine Empire decision no longer requires Greece to be a GP.
* Gave South Slavic countries and Greece a new decision that allows them to assimilate pops even if they are located in their own core provinces e.g. Turkish pops in Turkish cores will assimilate.

###### Germany:
* Added Kaiser Wilhelms Land decision that annexes Northern New Guinea and Nauru once colonial negotiations is invented. Nauru is a sulfur producing state with a strategic goods modifier that adds 50% throughput to sulfur production.

###### Southeast Asia:
* Added Rama V's Reforms decision to Siam.
* Added Claim Greater Burma decision to Burma.
* Created Indochina formable country.

###### Central Asia:
* Created new starting nations Maimana in northern Afghanistan and Chitral in northern Punjab.

###### Sikh Empire:
* Created Embrace Minorities decision that adds Pashtun, Sindi, and Baluchi as accepted cultures.

###### India:
* All Indian countries start with the Caste System modifier which lowers education efficiency and can be removed by passing Universal Suffrage and All Allowed Rights.

###### China:
* Opium War changed from a decision to an event.
* Owning a Chinese treaty port triggers an event at the start of each year that initially gives 36500 pounds. The amount given increases every 10 years. Owning more than one treaty port does not grant additional income.

###### United Kingdom:
* Replaced Joseph Tetley & Company with Blockow, Vaughan & Co.

###### Canada:
* Replaced Hudson Bay Company with MacMillan Bloedel Limited.

###### Chile:
* New Company: Compañía de Salitres y Ferrocarril de Antofagasta

###### British Raj:
* New Company: East India Company
