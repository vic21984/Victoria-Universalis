...
country = {
	...
	GOLD_TO_CASH_RATE = 4, -- Amount of money generated per gold unit
	...
	PROVINCE_OVERSEAS_PENALTY = 0, -- Each province req. this many goods flagged as overseas penalty 
	...
	TECH_YEAR_SPAN = 120,
	TECH_FACTOR_VASSAL = 0.9, -- cost reduction factor if overlord has research the tech
	MAX_SUPPRESSION = 100,
	...
	NAVAL_BASE_SUPPLY_SCORE_BASE = 12.5, -- base value that is powered by level of naval base. Determines the naval supplying capabilities.
	NAVAL_BASE_SUPPLY_SCORE_EMPTY = 1, -- min value for coastal provinces with no naval base.
	NAVAL_BASE_NON_CORE_SUPPLY_SCORE = 0.375, -- modifier for supply score for naval bases that are not in core provinces.
	...
	COLONIZATION_INFLUENCE_COST = 30,
	COLONIZATION_EXTRA_GUARD_COST = 10,
	COLONIZATION_RELEASE_DOMINION_COST = 30,
	COLONIZATION_CREATE_STATE_COST = 100,
	COLONIZATION_CREATE_PROTECTORATE_COST = 3,
	COLONIZATION_CREATE_COLONY_COST = 2, -- per province
	COLONIZATION_COLONY_STATE_DISTANCE = 50,
...
economy = {
	...
	GOODS_FOCUS_SWAP_CHANCE = 10, -- Percent increased chance that artisan wants to change goods independantly of how well he is doing presently
	...
	FACTORY_UPGRADE_EMPLOYEE_FACTOR = 0.5, -- determines how close to the employee limit we need to be before "upgrade all" will upgrade/expand a given factory (1 = 100%).
	...
	RGO_SUPPLY_DEMAND_FACTOR_FIRE = 0.04,		-- how fast pops are Fired when there is a low demand
	...
	FACTORY_PURCHASE_DRAWDOWN_FACTOR = 0.1		-- the % a factory will reduce its input purchases each day if it did not sell all its goods (also used for scaling up production if all goods are sold)
},
...
military = {
	...
	COMBATLOSS_WAR_EXHAUSTION = 6, -- base war exhastion in combat
	...
	AI_LIGHTSHIP_PROPORTION = 0.2, -- fraction of ships in the navy that should be cruisers, frigates etc
	AI_TRANSPORT_PROPORTION = 0.7, -- fraction of ships in the navy that should be should be transports
	AI_CAVALRY_PROPORTION = 0.1, -- fraction of brigades that should be cavalry
	AI_SUPPORT_PROPORTION = 0.45, -- fraction of brigades that should be artillery and other support units
	...
	NAVAL_LOW_SUPPLY_DAMAGE_SUPPLY_STATUS = 0.15, -- how little supply is acceptable before getting damage to STR
...
diplomacy = {
	...
	ASKMILACCESS_DIPLOMATIC_COST = 9,
	...
	TENSION_FROM_MOVEMENT = 0.2, -- tension per 1000 in nationalist movement
	TENSION_FROM_MOVEMENT_MAX = 1, -- max total value no matter the size of the movement
	AT_WAR_TENSION_DECAY = -0, -- for each potentially interested GP at war (value is if all possible GPs are at war) 
	...
	RANK_1_TENSION_DECAY = -0, -- extra flashpoint tension decay for GPs
